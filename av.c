...
int AVFrame_queue_put(bool queue, AVFrame *frame, VideoState *is) {
	...
	/*
	if (is->audio_frame_queue == NULL){
		printf("is->audio_frame_queue == NULL\n");
		return -1;
	}
	*/
	printf("before1\n");
...
int AVFrame_queue_get(bool queue, AVFrame *frame, int block, VideoState *is)
{
	...
	/*
	if (is->audio_frame_queue == NULL){
		printf("AVFrame_queue_get is->audio_frame_queue == NULL\n");
		return -1;
	}
	*/
	printf("before\n");
...
int video_frame_to_queue(uint16_t width, uint16_t height, const uint8_t *y, const uint8_t *u, const uint8_t *v, int32_t ystride, int32_t ustride, int32_t vstride, VideoState *is) {
	...
	printf("check22\n");
	/*
	memcpy(frame->data[0], y, abs(ystride) * frame->height);
	memcpy(frame->data[1], u, abs(ustride) * frame->height / 2);
	memcpy(frame->data[2], v, abs(vstride) * frame->height / 2);
	printf("check22\n");
	*/
