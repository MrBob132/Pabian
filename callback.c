...
{
	INFO("in tox_events_handle_file_chunk_request()")
	INFO("friend_number:%u , file_number:%u ", friend_number, file_number)
...
{
	//INFO("in tox_events_handle_file_recv_chunk()")
...
{
	INFO("in tox_events_handle_file_recv_control()")
	struct File_Transfer *t = gettransfer(friend_number, file_number);
	if (t){
...
{
	INFO("in tox_events_handle_file_recv()")
	INFO("friend_number:%u , file_number:%u ", friend_number, file_number)
    ...
    if (f) {
    	...
    	INFO("path: |%s|", path)
    	//strcat(path, DOWNLOAD_DICTIONARY);
...
{
	INFO("in audio_bit_rate_cb audio_bit_rate: %u", audio_bit_rate);
	struct Friend *f = getfriend(friend_number);
	struct Call *c = getcall(friend_number);
...
{
	INFO("in video_bit_rate_cb video_bit_rate: %u", video_bit_rate);
	struct Friend *f = getfriend(friend_number);
	struct Call *c = getcall(friend_number);
	Toxav_Err_Bit_Rate_Set err;
	if (f && c){
